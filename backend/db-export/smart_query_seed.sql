...
CREATE TABLE `sq_conversation` (
  ...
  `scenario` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '会话绑定场景编码，刷新页面恢复用',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted` tinyint NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
...
CREATE TABLE `sq_mining_model` (
  ...
  `algorithm_version` int DEFAULT NULL COMMENT '固化的算法版本',
  ...
  `user_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '归属用户ID（多租户隔离，admin 可见全部）',
...
CREATE TABLE `sq_model_execution` (
  ...
  `execution_kind` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'TRAIN',
  ...
  `algorithm_id` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `algorithm_version` int DEFAULT NULL,
...
CREATE TABLE `sq_prediction_result` (
  ...
  `trigger_type` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'manual' COMMENT 'manual/schedule',
  `model_execution_id` bigint DEFAULT NULL COMMENT '关联正式模型执行记录',
...
CREATE TABLE `sq_scenario` (
  ...
  `data_source_id` bigint DEFAULT NULL COMMENT '场景锁定数据源；NULL=用户自由切换',
  `schema_name` varchar(128) DEFAULT NULL COMMENT '数据库schema标识（仅配置元数据，不切换连接）',
  `allowed_tables` text COMMENT '表白名单CSV；NULL=该数据源全部表可见',
  `prompt_override` text COMMENT '场景级system prompt覆盖（预留扩展点）',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted` tinyint DEFAULT '0',
  PRIMARY KEY (`id`),
