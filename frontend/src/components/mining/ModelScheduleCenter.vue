<template>
  <section class="schedule-center">
    <div class="schedule-intro">
      <div>
        <span class="eyebrow">PRODUCTION MODELS</span>
        <h3>模型成品调度</h3>
        <p>集中配置已发布模型的重训与预测计划；点击模型成品可将其加入统一流程编排。</p>
      </div>
      <div class="permission-note">
        <strong>权限边界</strong>
        <span>{{ props.isAdmin ? '管理员可查看和调度全部模型成品' : '当前账号仅可查看和调度本人模型成品' }}</span>
        <small>进入流程编排前仍需通过机器学习算子版本审批</small>
      </div>
    </div>
    ...
    <div class="schedule-toolbar">
      ...
      <div class="toolbar-stat"><strong>{{ enabledCount }}</strong><span>调度中</span></div>
    ...
    <div v-else-if="!finishedModels.length" class="center-state empty-state">
      <span>◇</span>
      <h4>暂无已发布的模型成品</h4>
      <p>请先在模型流水线完成训练、评估与发布；发布后的模型会自动进入调度中心。</p>
    </div>
      ...
      <article v-for="model in filteredModels" :key="model.id" class="artifact-card" tabindex="0"
        ...
        <header>
          <div class="artifact-title">
            ...
            <div><strong>{{ model.name }}</strong><small>模型 #{{ model.id }} · v{{ model.version }}</small></div>
          </div>
          <el-tag :type="model.scheduleEnabled ? 'success' : 'info'" effect="plain">
            {{ model.scheduleEnabled ? '调度运行中' : '未启用调度' }}
          </el-tag>
        </header>
        ...
        <div class="artifact-status">
          <div>
            <span>调度策略</span>
            <strong>{{ scheduleDescription(model) }}</strong>
          </div>
          <div>
            <span>下次运行</span>
            <strong>{{ model.scheduleEnabled ? props.formatDate(model.nextRunAt) : '-' }}</strong>
          </div>
          <div>
            <span>编排授权</span>
        ...
        <footer>
          ...
          <el-button size="small" @click.stop="emit('configure', model)">调度配置</el-button>
...
const emit = defineEmits(['configure', 'openDag'])
const search = ref('')
...
const enabledCount = computed(() => finishedModels.value.filter(model => model.scheduleEnabled).length)
...
function scheduleDescription(model) {
  if (!model.scheduleEnabled) return '按需执行'
  const mode = model.scheduleMode === 'train' ? '定期重训' : '定期预测'
  return `${mode} · ${props.scheduleIntervalLabel(model)}`
}
function ownerLabel(model) {
  return props.isAdmin ? `归属用户 #${model.userId || '-'}` : '仅本人可调度'
}
</script>
